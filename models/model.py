...
class Graphormer(pl.LightningModule):
    ...
    def forward(self, batched_data, perturb=None):
        ...
        reg_output,weight=self.reg_head(batched_data,output)  #fine tune with reg task.
        reg_output=reg_output[np.arange(len(batched_data)),batched_data.task_num]
        return reg_output

    def training_step(self, batched_data, batch_idx):
        pred_r=self(batched_data)  #fine tune with reg task
        y_r=batched_data.y
        loss=self.loss_fn(pred_r,y_r)
        return loss
    ...
    def validation_epoch_end(self, outputs):
        y_pred_r=torch.cat([i['y_pred_r'] for i in outputs])
        y_true_r=torch.cat([i['y_true_r'] for i in outputs])
        input_dict = {
    ...
    def test_step(self, batched_data, batch_idx):
        pred_r=self(batched_data)  #fine tune with clf task
        y_r=batched_data.y
        return {
            'y_pred_r': pred_r,
            'y_true_r': y_r,
    ...
    def test_epoch_end(self, outputs):
        y_pred_r=torch.cat([i['y_pred_r'] for i in outputs])
        y_true_r=torch.cat([i['y_true_r'] for i in outputs])
        input_dict = {"y_true_r":y_true_r,"y_pred_r":y_pred_r}
